...
import { Loader2, AlertTriangle, ArrowLeft, MapPin, Calendar, Activity, AlertCircle } from "lucide-react"
...
export default function OutbreakDetailPage() {
  ...
  if (!outbreak) {
    return (
      <div className="container flex flex-col items-center justify-center min-h-[60vh] gap-2">
        ...
        <p className="text-muted-foreground">The requested outbreak could not be found</p>
  ...
  const totalCases = outbreak.affectedAreas.reduce((sum, area) => sum + area.caseCount, 0)
  ...
  return (
    <div className="container py-6">
      <Button variant="ghost" className="mb-6 pl-0 flex items-center gap-2" onClick={() => router.push("/outbreaks")}>
      ...
      <Card className={`border-l-4 ${getSeverityColor()}`}>
        <CardHeader>
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              <CardTitle className="text-2xl font-bold">{outbreak.disease}</CardTitle>
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-3xl font-bold">{outbreak.affectedAreas.length}</div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2">
              ...
              <CardContent>
                <div className="text-3xl font-bold">
                  {Math.ceil((new Date().getTime() - new Date(outbreak.startDate).getTime()) / (1000 * 60 * 60 * 24))}{" "}
                  days
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Geographic distribution of {outbreak.disease} cases</CardDescription>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Breakdown of cases by area</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <div key={index}>
                    ...
                    <div className="w-full bg-secondary h-2 rounded-full">
                      <div
                        ...
                        style={{
                          width: `${(area.caseCount / Math.max(...outbreak.affectedAreas.map((a) => a.caseCount))) * 100}%`,
                        }}
                      ></div>
                    </div>
                    {index < outbreak.affectedAreas.length - 1 && <Separator className="my-4" />}
                  </div>
                ))}
              </div>
            </CardContent>
