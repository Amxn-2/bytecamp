  ...
  useEffect(() => {
    ...
    const initializeMap = async () => {
      try {
        ...
        // Create new map
        const map = L.map(mapRef.current as HTMLElement).setView([center?.latitude || 19.076, center?.longitude || 72.8777], zoom)
      ...
      try {
        ...
        sensors.forEach((sensor) => {
          const marker = L.circleMarker([sensor.location.latitude, sensor.location.longitude], {
            radius: 8,
            fillColor: sensor.type === "air" ? "#ff5252" : sensor.type === "water" ? "#2196f3" : "#ffc107",
            fillOpacity: 0.8,
            color: "white",
            weight: 1,
          })
        ...
        outbreaks.forEach((outbreak) => {
          outbreak.affectedAreas.forEach((area) => {
            ...
            const marker = L.circleMarker([area.location.latitude, area.location.longitude], {
        ...
        if (markersRef.current.length > 0) {
          ...
          leafletMapRef.current.fitBounds(group.getBounds(), { padding: [30, 30] })
        }
  ...
  return (
    <Card className="w-full h-full overflow-hidden">
      <div
        ...
        style={{ zIndex: 0 }} // Ensure proper z-index
      />
    </Card>
  )
}
